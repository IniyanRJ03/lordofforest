...
const VENUES = [
  { title: "Family Room with Garden View 1", image: "/images/room 1/615134276.jpg" },
  { title: "Family Room with Garden View 2", image: "/images/room 2/615134289.jpg" },
];
    ...
    <section className="relative w-full h-[50vh] min-h-[300px] sm:h-[60vh] sm:min-h-[400px] md:h-[70vh] md:min-h-[500px] lg:h-[80vh] lg:min-h-[600px] flex items-center justify-center overflow-hidden">
      ...
      <div className="relative z-20 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col justify-center h-full pt-16 sm:pt-20 lg:pt-24">
        <motion.span 
          ...
          className="uppercase text-xs sm:text-sm tracking-widest text-gray-200 font-semibold mb-2 sm:mb-3 block"
        >
          Our Accommodations
        </motion.span>
        <motion.h1 
        ...
        {/* Venues row (room types) - Hidden on mobile */}
        <motion.div 
          initial={{ opacity: 0, y: 40 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          {VENUES.map((venue, i) => (
            <motion.div
              ...
              className="rounded-xl sm:rounded-2xl overflow-hidden shadow-lg bg-white/10 backdrop-blur-sm w-48 min-w-[180px] sm:w-56 sm:min-w-[200px] md:w-64 md:min-w-[240px] lg:w-72 lg:min-w-[260px] flex flex-col cursor-pointer group flex-shrink-0"
            >
